...
@Service
public class DailyEmployeeScoreAnalyticsReportKafkaMessageStructImpl extends AbstractKafkaMessageStruct<DailyEmployeeScoreAnalyticsReport> {

    private static final ProducerTargetObjectType PRODUCER_TARGET_OBJECT_TYPE = ProducerTargetObjectType.DAILY_EMPLOYEE_SCORE_ANALYTICS_REPORT;

    protected DailyEmployeeScoreAnalyticsReportKafkaMessageStructImpl(MessageMetaInfoStruct metaInfoStruct) {
    ...
    @Override
    protected String defineTargetObjectId(DailyEmployeeScoreAnalyticsReport model) {
        return model.getUuid().toString();
    }

    @Override
    protected Class<DailyEmployeeScoreAnalyticsReport> definePayloadType() {
        return DailyEmployeeScoreAnalyticsReport.class;
    }

    @Override
    public String defineKey(DailyEmployeeScoreAnalyticsReport model) {
